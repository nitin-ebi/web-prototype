...
class GenotypeView extends Component {
    ...
    _fetchConsequenceRanking() {
        this.setState({isLoaded: false});
        fetch(`https://rest.ensembl.org/info/variation/consequence_types?content-type=application/json&rank=1`)
            .then(response => response.json())
            .then(consequences => {
                this.consequenceRankMap = Object.fromEntries(
                    consequences.map(consequence => [consequence.SO_accession, parseInt(consequence.consequence_ranking)])
                );
            });
    }
...
export default hot(module)(GenotypeView);
